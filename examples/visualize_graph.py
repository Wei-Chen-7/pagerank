...
from _common import ACCENT, GRID, INK, MUTED, apply_style, figure_path
...
def main() -> None:
    ...
    nx.draw_networkx_edges(
        nxg, pos, ax=ax, edge_color=MUTED, width=1.2,
        arrowsize=14, arrowstyle="-|>", alpha=0.7,
        connectionstyle="arc3,rad=0.06", node_size=sizes,
    )
    nodes = nx.draw_networkx_nodes(
        nxg, pos, ax=ax, nodelist=order, node_size=sizes,
        node_color=colors, cmap="viridis", edgecolors="white", linewidths=1.5,
    )
    # Label only the nodes that matter to the story; leave the crowd unlabeled.
    labels = {n: n for n in ("guru", "insight", "megablog")}
    nx.draw_networkx_labels(nxg, pos, labels=labels, ax=ax, font_size=11,
